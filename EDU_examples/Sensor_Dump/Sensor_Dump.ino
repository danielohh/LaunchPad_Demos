...
void loop() { 
    ...
    myScreen.setFontSolid(true);
    myScreen.setFontSize(0);
    sprintf(buf, "Y-Axis: %d", analogValue);
    myScreen.gText(4, 24, buf, greenColour);    
    ...
    myScreen.setFontSolid(true);
    myScreen.setFontSize(0);
    sprintf(buf, "Z-Axis: %d", analogValue);
    myScreen.gText(4, 34, buf, blueColour);    
    ...
    if(analogValue >2048){ // check if tilting on Z axis in positive direction 
     ...
     myScreen.gText(4, 44, "Joystick Data:", whiteColour);
     joystickXState = analogRead(joystickX);
     joystickYState = analogRead(joystickY);
     joystickSelState = digitalRead(joystickSel);
     sprintf(buf, "X:%d ", joystickXState);
     myScreen.gText(4, 54, buf, redColour);
     sprintf(buf, "Y:%d ", joystickYState);
     myScreen.gText(4, 64, buf, greenColour);
     sprintf(buf, "Sel:%d", joystickSelState);
     myScreen.gText(4, 74, buf, blueColour);
     sprintf(buf,"");
